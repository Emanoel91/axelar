...
grouped["avg_volume_per_tx"] = (
    grouped["volume"] /
    grouped["num_txs"].replace(0, pd.NA)
)

# =====================================================
# AVG VOLUME PER TX OVER TIME
# =====================================================

fig_avg_volume_tx = go.Figure()

fig_avg_volume_tx.add_trace(
    go.Scatter(
        x=grouped["period"],
        y=grouped["avg_volume_per_tx"],
        mode="lines",
        fill="tozeroy",
        name="Avg Volume per Transaction"
    )
)

fig_avg_volume_tx.update_layout(
    title=f"{token_symbol} Avg Volume per Transaction Over Time",
    template="plotly_dark",
    hovermode="x unified",
    yaxis_title="Avg Volume per Transaction",
    xaxis_title="",
    height=500
)
# =====================================================
